...
class CreateGroup(MethodView):
    ...
    def post(current_user, self):
        """
        Register a user, generate their token and \
        ...
        if not status == "true":
            return jsonify({"status": 401, "message": "You don't \
              have admin rights to execute this resource"})

        if request.content_type == 'application/json':
            if 'name' in request.json and 'role' in request.json:
                data = request.get_json()
                group_name = data.get('name')
                group_role = data.get('role')
                group_owner = current_user

                if isinstance(group_role, str) and isinstance(group_role, str):

                    group = Group.get_group_by_groupname(group_name)
                    if not group:
                        Group(group_name=group_name, group_role=group_role,
                              group_owner=group_owner).save_group()
                        cur = conn.cursor()
                        sql1 = """
            ...
            """
                    ...
                    return jsonify({
                        "status": 400,
                        "error": "Failed, Groupname already exists"
                    }), 400
                return jsonify({
                    "status": 400,
                    "error": "groupname or grouprole or groupowner\
                         should be a string"}), 400
            return jsonify({
                "status": 400,
                "error": "groupname or grouprole is missing"
            }), 400
        return jsonify({
            "status": 400,
            "error": "Content type must json"
        }), 400
    ...
    def put(current_user, self, group_id):
        ...
        if not isinstance(new_group_name, str) or not new_group_name:
            return jsonify({
                'status': 400,
                'message': 'The group name must a string and cannot be empty'})
...
class UserGroup(MethodView):
    ...
    def delete(current_user, self, group_id, user_id):
        ...
        try:
            ...
            return jsonify({
                'status': 404,
                'error': 'Invalid group_id or user_id'})
        ...
        if groups and users:
            ...
            return jsonify({
                'status': 200,
                'data': [{
                    'message': 'User deleted from the successfully'
                }]
            })
        return jsonify({
            'status': 200,
        ...
        if request.content_type == 'application/json':
            if 'subject' in request.json and 'message' in request.json:
                ...
                if isinstance(subject, str) and isinstance(message, str):
                    ...
                    cur = conn.cursor()
                    sql = """
                  INSERT INTO group_messages (senderId, groupId, \
                      subject, message, parentMessageId, createdOn) 
                          VALUES (%s, %s, %s, %s, %s, %s)
              """
                    createdOn = str(datetime.datetime.now())
                    cur.execute(sql, (current_user, groupId, subject,
                                      message, current_user, createdOn))
                    conn.commit()
